...
public class PointsRecordServiceImpl extends ServiceImpl<PointsRecordMapper, PointsRecord> implements IPointsRecordService {
    ...
    public List<PointsStatisticsVO> queryMyPointsToday() {
        ...
        for (PointsRecord p : list) {
            ...
            // 【修复核心】使用枚举的 of() 方法将 Integer 转换为 PointsRecordType
            // p.getType() 返回的是 Integer，PointsRecordType.of() 会将其转为枚举对象
            PointsRecordType typeEnum = PointsRecordType.of(p.getType());

            if (typeEnum != null) {
                vo.setType(typeEnum.getDesc());
                vo.setMaxPoints(typeEnum.getMaxPoints());
            } else {
                // 兜底处理：防止数据库中存在未知的类型值导致空指针
                vo.setType("未知类型");
                vo.setMaxPoints(0);
            }
